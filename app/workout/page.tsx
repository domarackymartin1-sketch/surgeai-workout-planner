...
export default function WorkoutPage() {
  ...
  const handleCreatePlan = () => {
    ...
    addPlan({
      name: newPlanName,
      muscleGroups: ['Vlastný'],
      exercises: [{ exerciseId: 'bench-press', sets: 3, targetReps: 10 }],
      estimatedMinutes: 45,
    });
    setNewPlanName('');
    setShowModal(false);
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/80 p-4">
          <div className="w-full max-w-lg rounded-card bg-bg-card p-6">
            <h3 className="mb-4 font-display text-xl font-bold">Nový plán</h3>
            <input
              type="text"
              placeholder="Názov plánu"
              value={newPlanName}
              onChange={(e) => setNewPlanName(e.target.value)}
              className="mb-4 w-full rounded-btn bg-bg-surface px-4 py-3 outline-none"
              autoFocus
            />
            <div className="flex gap-2">
              <button
                ...
                className="touch-manipulation flex-1 rounded-btn border border-text-dim py-3 text-text-muted"
              >
                Zrušiť
              </button>
              <GreenButton onClick={handleCreatePlan} className="flex-1">
                Vytvoriť
              </GreenButton>
            </div>
          </div>
