    ...
    <section className="mb-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button type="button" className="glass flex h-9 w-9 items-center justify-center rounded-xl text-white/50">
            <Pencil size={16} />
          </button>
          <button type="button" className="glass flex h-9 w-9 items-center justify-center rounded-xl text-white/50">
            <Plus size={16} />
              ...
              {weekDateStrs.map((dateStr) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {done && <span className="h-2 w-2 rounded-sm bg-accent-green" />}
                    </span>
                  </div>
                );
              })}
        ...
        <div className="glass-card mt-4 p-5">
          ...
          <div className="grid grid-cols-7 gap-2">
            {weekDays.map((date, i) => {
              ...
              const done = isCompleted(activeHabit, dateStr);
              const dayName = ['Po', 'Ut', 'St', 'Št', 'Pi', 'So', 'Ne'][i];
              return (
                <button
                  key={dateStr}
                  type="button"
                  onClick={() => toggleHabitForDate(activeHabit, dateStr)}
                  className="flex flex-col items-center gap-1.5"
                >
                  <span className="text-[10px] text-white/35">{dayName}</span>
                  <span
                  ...
                  >
                    {done && <span className="h-3 w-3 rounded-md bg-accent-green" />}
                  </span>
                </button>
              );
            })}
